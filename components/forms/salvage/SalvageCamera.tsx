...
export default function SalvageCamera({
  open,
  onClose,
  onAdd,
  maxCount = 30,
  ...
  async function capture() {
    ...
    if (!blob) return;
    const safePrefix = (downloadPrefix || "salvage").replace(
      /[^a-zA-Z0-9_-]/g,
      "-"
    );
    const filename = `${safePrefix}-${Date.now()}.jpg`;
    const file = new File([blob], filename, { type: "image/jpeg" });
    // No immediate download per capture; accumulate and zip on Done
      ...
      <div className="relative w-full sm:w-[98%] max-w-none sm:max-w-2xl md:max-w-3xl lg:max-w-5xl xl:max-w-6xl h-[100dvh] sm:h-[96dvh] max-h-[100dvh] sm:max-h-[96dvh] overflow-hidden flex flex-col rounded-none sm:rounded-2xl border-0 sm:border border-rose-200/30 bg-black/30 ring-0 sm:ring-1 ring-black/50 shadow-2xl">
          ...
          >
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 w-full">
              <button
                type="button"
                onClick={exitAndClose}
                className="h-12 w-full inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 text-sm font-semibold text-white ring-1 ring-white/10 hover:bg-blue-500 cursor-pointer"
                aria-label="Close"
              >
              ...
              <button
                type="button"
                onClick={done}
                className="h-12 w-12 inline-flex items-center justify-center rounded-2xl bg-gradient-to-b from-rose-500 to-rose-600 text-white shadow-[0_6px_0_0_rgba(190,18,60,0.45)] ring-2 ring-rose-300/60 hover:from-rose-400 hover:to-rose-600 focus:outline-none cursor-pointer"
                aria-label="Done"
                title="Done"
              >
              ...
              <button
                type="button"
                onClick={() => {}}
                className="h-12 w-full inline-flex items-center justify-center gap-2 rounded-xl bg-green-600 px-3 text-sm font-semibold text-white ring-1 ring-white/10 hover:bg-green-500 cursor-default"
                aria-label="Next"
              >
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 w-full">
              <button
                type="button"
                onClick={capture}
                className="h-11 inline-flex cursor-pointer items-center justify-center gap-2 rounded-full bg-gradient-to-b from-rose-500 to-rose-600 px-3 text-sm font-semibold text-white shadow-[0_5px_0_0_rgba(190,18,60,0.45)]"
                title="Capture"
              >
              ...
              <button
                ...
                className="h-11 inline-flex cursor-pointer items-center justify-center gap-2 rounded-full bg-white/90 border border-gray-200 px-3 text-sm font-semibold text-gray-800 shadow hover:bg-white"
              >
              ...
              <input
                ...
                multiple
                className="sr-only"
                onChange={(e) => addManual(e.target.files)}
              />
            </div>
          </div>
        ...
        <div className="border-t border-gray-800 bg-black/60 text-white/90 p-2">
          <div className="flex items-center gap-2 overflow-x-auto">
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => removeAt(i)}
                    className="absolute right-1 top-1 rounded-full bg-black/70 p-1 text-white shadow-lg hover:bg-black/80 transition"
                    aria-label="Remove"
                  >
            ...
            {files.length === 0 && (
              <div className="text-xs text-white/70">No images yet</div>
            )}
          </div>
        </div>
      </div>
